...
#define LSM6DSV32X_ADDR 0x6A

static w_status_t timer_get_ms_custom_fake(uint32_t *ms) {
    if (NULL == ms) {
        return W_INVALID_PARAM;
    }

    *ms = 1234; // arbitrary deterministic timestamp
    return W_SUCCESS;
}
...
static w_status_t i2c_read_reg_custom_fake_false_whoami(
    i2c_bus_t bus, uint8_t device_addr, uint8_t reg, uint8_t *data, uint8_t len
) {
    (void)bus;
    (void)device_addr;
    (void)reg;
    (void)len;
    *data = 0x72; // expected WHO_AM_I for LSM6DSV32X
    return W_SUCCESS;
}
...
TEST_F(Lsm6dsv32xTest, InitSuccess) {
    i2c_write_reg_fake.return_val = W_SUCCESS;
    i2c_read_reg_fake.custom_fake = i2c_read_reg_custom_fake_whoami;

    EXPECT_EQ(lsm6dsv32x_init(), W_SUCCESS);
}

TEST_F(Lsm6dsv32xTest, InitFailByI2CWrite) {
...
TEST_F(Lsm6dsv32xTest, InitFailBySanityCheck) {
    i2c_write_reg_fake.return_val = W_FAILURE;
    i2c_read_reg_fake.custom_fake = i2c_read_reg_custom_fake_false_whoami;
...
TEST_F(Lsm6dsv32xTest, GetGyroAccDataEvenIfNotFresh) {
    ...
    // assume called isr but not call dma callback yet
...
TEST_F(Lsm6dsv32xTest, GetGyroAccDataDataReady) {
    // Arrange
    // set up init
    i2c_write_reg_fake.return_val = W_SUCCESS;
    i2c_read_reg_fake.custom_fake = i2c_read_reg_custom_fake_whoami;
    ...
    // got dma callback
    lsm6dsv32x_int1_isr_handler();
    lsm6dsv32x_dma_complete_handle(&hi2c1);
