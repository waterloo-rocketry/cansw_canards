...
static void system_init_task(void *arg) {
	...
	log_text(10, "SystemInit", "All tasks created successfully.");// its blinky now

	uint8_t i = 0;
	TickType_t last_wake_time = xTaskGetTickCount();
	float64_t gyro_data = 0;
	vector3d_t accel_data = {0};
	...
	while (1) {
		...
		ad_status = ad_breakout_board_get_accel_data(&acc_data, &timestamp_ms);
		if (ad_status != W_SUCCESS) log_text(10, "SystemInit", "AD BOARD ERROR.");


		log_text(10, "SystemInit", "AD Accel timestamp %"PRIu32, 
					timestamp_ms);
		log_text(10, "SystemInit", "AD ACCEL x %lf, y %lf, z %lf", 
			acc_data.x, acc_data.y, acc_data.z);



		ad_status = ad_breakout_board_get_gyro_data(&gyro_data, &timestamp_ms);
		if (ad_status != W_SUCCESS) log_text(10, "SystemInit", "AD BOARD ERROR.");

		log_text(10, "SystemInit", "AD Gyro timestamp %"PRIu32, 
					timestamp_ms);
		log_text(10, "SystemInit", "AD GYRO  z %lf", 
			gyro_data);
