...
static const matrix3d_t J = {
	.array = {{0.46, 0, 0}, {0, 49.5, 0}, {0, 0, 49.5}}}; // inertia matrix of the rocket
static const matrix3d_t J_inv = {
	.array = {{1 / 0.46, 0, 0},
			  {0, 1 / 49.5, 0},
			  {0, 0, 1 / 49.5}}}; // inverse of inertia matrix of the rocket (J^-1 in matlab)
static vector3d_t g = {
	.array = {-9.81, 0, 0}}; // gravitational acceleration in the geographic inertial frame
...
static matrix3d_t tilde(const vector3d_t *vector) {
	...
	return (matrix3d_t){.array = {{0, -vector->z, vector->y},
								  {vector->z, 0, -vector->x},
								  {-vector->y, vector->x, 0}}};
}
...
void model_dynamics_jacobian(double pData_dynamic_jacobian[SIZE_STATE * SIZE_STATE],
	...
	write_pData(pData_dynamic_jacobian,
				0,
				0,
				SIZE_QUAT,
				SIZE_QUAT,
				&q_q.flat[0]); // J_x(1:4,1:4) = q_q; % column q (attitude)
	write_pData(pData_dynamic_jacobian,
				0,
				4,
				SIZE_QUAT,
				SIZE_VECTOR_3D,
				&q_w.flat[0]); // J_x(1:4, 5:7) = q_w; % column w (rates)
	...
	const matrix3d_t J_inv_scaled = {.array = {{1 / 0.46 * (dt), 0, 0},
											   {0, 1 / 49.5 * (dt), 0},
											   {0, 0, 1 / 49.5 * (dt)}}}; // dt * param.Jinv
	...
	write_pData(pData_dynamic_jacobian,
				...
				SIDE_MATRIX_3D,
				SIDE_MATRIX_3D,
				&w_w.flat[0]); // J_x(5:7,5:7) = w_w; % column w
	write_pData(pData_dynamic_jacobian,
				4,
				7,
				SIDE_MATRIX_3D,
				SIDE_MATRIX_3D,
				&w_v.flat[0]); // J_x(5:7,8:10) = w_v; % column v
	write_pData(pData_dynamic_jacobian,
				4,
				11,
				SIZE_VECTOR_3D,
				SIZE_1D,
				&w_cl.array[0]); // J_x(5:7,12) = w_cl; % column Cl
	write_pData(pData_dynamic_jacobian,
				4,
				12,
				SIZE_VECTOR_3D,
				SIZE_1D,
				&w_delta.array[0]); // J_x(5:7,13) = w_delta; % column delta
	...
	write_pData(pData_dynamic_jacobian,
				...
				SIZE_VECTOR_3D,
				SIZE_QUAT,
				&v_q.flat[0]); // J_x(8:10,1:4) = v_q; % column q
	write_pData(pData_dynamic_jacobian,
				7,
				4,
				SIDE_MATRIX_3D,
				SIDE_MATRIX_3D,
				&v_w.flat[0]); // J_x(8:10,5:7) = v_w; % column w
	write_pData(pData_dynamic_jacobian,
				7,
				7,
				SIDE_MATRIX_3D,
				SIDE_MATRIX_3D,
				&v_v.flat[0]); //  J_x(8:10,8:10) = v_v; % column v
	...
	quaternion_rotate_jacobian(
		&r_q.flat[0],
		&q_inv,
		&(state->velocity)); // quaternion_rotate_jacobian(quaternion_inv(q), v)
	for (int i = 0; i < (SIZE_VECTOR_3D * SIZE_QUAT); i++) {
		r_q.flat[i] *= dt;
	...
	write_pData(pData_dynamic_jacobian,
				...
				SIZE_1D,
				SIZE_QUAT,
				&alt_q[0]); // J_x(11,1:4) = alt_q; % column q
	write_pData(pData_dynamic_jacobian,
				10,
				7,
				SIZE_1D,
				SIZE_VECTOR_3D,
				&alt_v.array[0]); // J_x(11,8:10) = alt_v; % column v
	write_pData(pData_dynamic_jacobian,
				10,
				10,
				SIZE_1D,
				SIZE_1D,
				&alt_alt); // J_x(11, 11) = alt_alt; % column alt
	...
	write_pData(pData_dynamic_jacobian,
				...
				SIZE_1D,
				SIZE_1D,
				&Cl_Cl); // J_x(12,12) = Cl_cl; % column Cl
	...
	write_pData(pData_dynamic_jacobian,
				...
				SIZE_1D,
				SIZE_1D,
				&delta_delta); // J_x(13,13) = delta_delta; % column delta
}
