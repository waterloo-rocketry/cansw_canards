...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Input may be a binary .BIN log, a wide CSV from logparse.py, or a "
            "folder of per-type CSV files from csvparse.py."
        )
    )
    parser.add_argument(
        "input_path", help=".BIN log, CSV file, or directory of per-type CSV files"
    )
    parser.add_argument(
        "--timestamp-scale",
        type=float,
    ...
    parser.add_argument(
        "--save-prefix",
        help=(
            "Prefix for output PNG files (default: the input log number/name in "
            "the input directory)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip running logcheck.py's rate-compliance/dropped-buffer/timing-gap report "
            "(only applies to .BIN input; ignored for CSV/directory input)"
        ),
    )
    parser.add_argument(
        "--textlog",
        help="Companion .TXT log to pass through to logcheck.py for flight-phase boundaries",
    )
    return parser.parse_args()


def _parse_bin(input_path: Path) -> Path:
    logparse_path = Path(__file__).with_name("logparse.py")
    subprocess.run(
        [sys.executable, str(logparse_path), str(input_path)],
        check=True,
    )

    csv_path = Path(f"{input_path}-parsed.csv")
    if not csv_path.is_file():
        raise RuntimeError(f"logparse.py did not generate the expected file: {csv_path}")

    return csv_path


def _run_logcheck(input_path: Path, textlog: str | None) -> None:
    """Run logcheck.py's compliance report (no plots -- we already made our own)."""
    logcheck_path = Path(__file__).with_name("logcheck.py")
    cmd = [sys.executable, str(logcheck_path), str(input_path), "--no-plots", "--no-show"]
    if textlog:
        cmd += ["--textlog", textlog]

    print(f"\nRunning {logcheck_path.name} ...")
    # logcheck.py exits non-zero on FAIL/WARN findings; that's a report, not a crash here.
    subprocess.run(cmd, check=False)


def _default_save_prefix(input_path: Path) -> Path:
    if input_path.is_dir():
        return input_path / input_path.name

    filename = input_path.name
    if filename.lower().endswith(".bin-parsed.csv"):
        log_name = filename[: -len(".bin-parsed.csv")]
    else:
        log_name = input_path.stem

    return input_path.parent / log_name
...
def main() -> None:
    ...
    is_bin = input_path.suffix.lower() == ".bin"

    if input_path.is_dir():
        type_map = load_from_csv_dir(input_path)
    elif is_bin:
        csv_path = _parse_bin(input_path)
        print(f"Parsed {input_path} into {csv_path}")
        type_map = load_from_wide_csv(csv_path)
    elif input_path.suffix.lower() == ".csv":
        type_map = load_from_wide_csv(input_path)
    else:
        raise ValueError("Input file must have a .BIN or .csv extension.")
    ...
    save_prefix = (
        ...
        else _default_save_prefix(input_path)
    )
    ...
    if is_bin and not args.no_check:
        _run_logcheck(input_path, args.textlog)
